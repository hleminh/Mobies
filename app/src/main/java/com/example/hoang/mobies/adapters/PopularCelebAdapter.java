    ...
    public class PopularCelebViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(PeopleModel peopleModel) {
            if (peopleModel.getGender() == 1)
                Picasso.with(context).load("http://image.tmdb.org/t/p/w342/" + peopleModel.getProfile_path()).centerInside().fit().placeholder(R.drawable.no_image_person_f_final_smaller).into(ivCbpImage);
            else if (peopleModel.getGender() == 2)
                Picasso.with(context).load("http://image.tmdb.org/t/p/w342/" + peopleModel.getProfile_path()).centerInside().fit().placeholder(R.drawable.no_image_person_m_final_smaller).into(ivCbpImage);
            else
                Picasso.with(context).load("http://image.tmdb.org/t/p/w342/" + peopleModel.getProfile_path()).centerInside().fit().placeholder(R.drawable.no_image_person_u_final_2_smaller).into(ivCbpImage);
